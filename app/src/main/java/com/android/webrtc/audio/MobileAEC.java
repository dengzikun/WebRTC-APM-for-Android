...
public class MobileAEC {
     ...
     *            FS_16000Hz} is set.
     */
    public MobileAEC(SamplingFrequency sampFreqOfData) throws Exception  {
        ...
        if(!nativeCreateAecmInstance()){
            throw new Exception("create aec failed!");
        }
    }
    ...
    public MobileAEC farendBuffer(short[] farendBuf, int numOfSamples)
        ...
        if (!nativeBufferFarend(farendBuf, numOfSamples))
            // TODO(billhoo) - create a custom exception instead of using java.lang.Exception
            throw new Exception(
    ...
    public void echoCancellation(short[] nearendNoisy, short[] nearendClean,
        ...
        if (!nativeAecmProcess(nearendNoisy, nearendClean, out,
                numOfSamples, delay))
            // TODO(billhoo) - create a custom exception instead of using java.lang.Exception
            throw new Exception(
    ...
    private void mInitAecmInstance(int SampFreq) {
        if (!mIsInit) {
            nativeInitializeAecmInstance(SampFreq);
     ...
     *          -1: error
     */
    private native boolean nativeInitializeAecmInstance(int samplingFrequency);
     ...
     *          -1: error
     */
    private native boolean nativeBufferFarend(short[] farend, int nrOfSamples);
     ...
     *          -1: error
     */
    private native boolean nativeAecmProcess(short[] nearendNoisy, short[] nearendClean, short[] out,
